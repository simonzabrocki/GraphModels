nodes = [
    {'type': 'variable',
     'unit': 'kcal/day',
     'id': 'TCDi',
     'name': 'Total calorie demand per food group',
     'in': ['P', 'FWi', 'FWCRi', 'FDKCi'],
     'computation': {'name': 'P*(FDKCi+FWi-FWCRi)',
                     'formula': None}
     },
    {'type': 'input',
     'unit': 'kcal/cap/day',
     'id': 'FWCRi',
     'name': 'Waste reduction policy consumption per food group',
     },
    {'type': 'input',
     'id': 'FWPPi',
     'unit': '%',
     'name': 'Waste reduction policy production per food group',
     },
    {'type': 'variable',
     'unit': 'kcal/cap/day',
     'id': 'FWi',
     'name': 'Food waste per food group',
     'in': ['FWPi_baseline', 'FWPPi'],
     'computation': {'name': 'FWPi_baseline * FWPPi',
                     'formula': None}
     },
    {'type': 'variable',
     'unit': 'kcal/cap/day',
     'id': 'FWPi_baseline',
     'name': 'Food waste baseline per food group',
     'in': ['FLOi', 'P', 'KKRi'],
     'computation': {'name': 'FLOi * KKRi * / P',
                     'formula': None}
     },
    {'type': 'input',
     'unit': 'x 1000 t',
     'id': 'FLOi',
     'name': 'Food losses per food group',
     },
    {'type': 'variable',
     'id': 'KKRi',
     'name': 'Kcal/kg ratio per food group',
     'unit': 'Kcal/kg',
     'in': ['FDKGi', 'FDKCi'],
     'computation': {'name': 'FDKGi / FDKCi',
                     'formula': lambda X: None}
     },
    {'type': 'input',
     'unit': 'kg/capita/yr',
     'id': 'FDKGi',
     'name': 'Kg food demand per year per food group',
     },
    {'type': 'input',
     'unit': 'kcal/capita/day',
     'id': 'FDKCi',
     'name': 'Kcal food demand per day per food group',
     },
    {'type': 'input',
     'unit': 'capita',
     'id': 'P',
     'name': 'Total population',
     },
    {'type': 'variable',
     'unit': '%',
     'id': 'SSRi',
     'name': 'Self-sufficiency ratio per food group',
     'in': ['FPi', 'FEi', 'FIi'],
     'computation': {'name': 'FPi / (FPi - FEi + FIi)',
                     'formula': lambda X: None}
     },
    {'type': 'input',
     'unit': '1000 t',
     'id': 'FPi',
     'name': 'Food production per food group',
     },
    {'type': 'input',
     'unit': '1000 t',
     'id': 'FEi',
     'name': 'Food exports per food group',
     },

    {'type': 'input',
     'unit': '1000 t',
     'id': 'FIi',
     'name': 'Food imports per food group',
     },

    {'type': 'variable',
     'unit': '1000 t',
     'id': 'FDi',
     'name': 'Total domestic food production per food group',
     'in': ['KKRi', 'SSRi', 'TCDi'],
     'computation': {'name': 'TCDi * SSRi / KKRi',
                     'formula': lambda X: None}


     },
    {'type': 'input',
     'unit': '1000 t',
     'id': 'FDi_baseline',
     'name': 'Feed demand per group',

     },
    {
        'type': 'variable',
        'id': 'FDTi_animal',
        'name': 'Total food demand per food group (animals)',
        'unit': 'x 1000 tonne',
        'in': ['FDi', 'OFi', 'FDi_baseline', 'SSRi'],
        'computation': {'name': 'SSRi_animal * (OFi_animal + FDi_baseline_animal) + FDi_animal',
                        'formula': None}
    },
    {'type': 'variable',
     'unit': '1000 t',
     'id': 'OFi',
     'name': 'Other food demand',
     'in': ['SDi', 'NFDi', 'PDi', 'RDi', 'SVi'],
     'computation': {'name': 'SDi + NFDi + PDi + RDi + SVi',
                     'formula': lambda X: None}
     },

    {'type': 'input',
     'unit': '1000 t',
     'id': 'SDi',
     'name': 'Seed demand per food group',
     },
    {'type': 'input',
     'unit': '1000 t',
     'id': 'NFDi',
     'name': 'Non-food demand per food group',
     },
    {'type': 'input',
     'unit': '1000 t',
     'id': 'PDi',
     'name': 'Processed demand per food group',
     },
    {'type': 'input',
     'unit': '1000 t',
     'id': 'RDi',
     'name': 'Residual demand per food group',
     },
    {'type': 'input',
     'unit': '1000 t',
     'id': 'SVi',
     'name': 'Stock variation per food group',
     },
    {'type': 'variable',
     'id': 'FDTi_crops',
     'name': 'Total food demand per food group (crops)',
     'unit': 'x 1000 tonne',
     'in': ['FDi', 'OFi', 'AFDi', 'SSRi'],
     'computation': {'name': 'SSRi_crop * (OFi_crop + AFDi) + FDi_crop',
                     'formula': lambda X: None}
     },
    {'type': 'variable',
     'unit': '1000 t',
     'id': 'AFDi',
     'name': 'Total animal feed demand per crop',
     'in': ['FMi', 'FD'],
     'computation': {'name': 'FMi * FD',
                     'formula': lambda X: None}
     },
    {'type': 'variable',
     'unit': '',
     'id': 'FMi',
     'name': 'Feed mix fraction',
     'in': ['FDi_baseline'],
     'computation': {'name': 'FDi_baseline / SUM(FDi_baseline)',
                     'formula': lambda X: None}
     },
    {'type': 'variable',
     'unit': '1ooo t',
     'id': 'FD',
     'name': 'Total crop-feed demand',
     'in': ['TAFDi', 'CRfd'],
     'computation': {'name': 'SUM(TAFDi)/CRfd',
                     'formula': lambda X: None}
     },
    {'type': 'input',
     'unit': 'kg',
     'id': 'TAFDi_baseline',
     'name': 'Total animal feed demand per animal',
     },
    {'type': 'parameter',
     'unit': '',
     'id': 'CRfd',
     'name': 'feed / forage distribution (correction)',
     'in': ['TAFDi_baseline', 'FDi_baseline'],
     'computation': {'name': 'SUM(TAFDi_baseline)/sum(FDi_baseline)',
                     'formula': lambda X: None}

     },
    {'type': 'variable',
     'unit': 'kg',
     'id': 'TAFDi',
     'name': 'Total animal feed demand per food group',
     'in': ['FCRi', 'AYi', 'ANPi'],
     'computation': {'name': '(FCRi* AYi * ANPi)',
                     'formula': lambda X: None}
     },
    {'type': 'parameter',
     'unit': 'kg DM feed/ kg EW',
     'id': 'FCRi',
     'name': 'Feed conversion ratio (kg/animal)',
     },
    {'type': 'input',
     'unit': 'kg/animal',
     'id': 'AYi',
     'name': 'Animal yields per animal group',
     },

    {'type': 'variable',
     'unit': 'heads',
     'id': 'ANPi',
     'name': 'Animals needed for production per food group',
     'in': ['FDi', 'AYi'],
     'computation': {'name': 'FDi_animal/AYi',
                     'formula': lambda X: None}

     },
    {'type': 'output',
     'id': 'FDTi',
     'name': 'Total food demand per group',
     'unit': 'x 1000 tonne',
     'in': ['FDTi_animal', 'FDTi_crops'],
     'computation': {'name': 'FDTi_animal, FDTi_crops',
                     'formula': lambda X: None}
     },

]
